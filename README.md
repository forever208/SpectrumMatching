...
## Step 0: Installation
```bash
conda create -n SDVAE python==3.9.13
...
In the [```stageq_vae_train.yaml```](configs/train_vae_celeba256.yaml) we define everything we need to train our model (there is a separate config for [VQVAE training](configs/stage1_vqvae_train.yaml))
